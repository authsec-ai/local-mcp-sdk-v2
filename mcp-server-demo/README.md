...
1. Place `mcp_oauth_sdk.py` and `another_mcp_server.py` in the same directory
2. Set the required OAuth environment variables (e.g., `OAUTH_SERVER`, `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, `OAUTH_SCOPES`)
3. Run the demo server: `python another_mcp_server.py`
4. Interact with it via an MCP client, following the "Usage" section above. You'll see the SDK's OAuth tools and the demo server's tools (`get_greeting`, `get_random_number`, `admin_action`)
